...
DNA::DNA()
{
    charCount = 0;
    lineCount = 0;
    squaredUsed = 0;//variable for calculating variance
    mean = 0;
    variance = 0;
    stdDev = 0;
    ...
    probA = 0; //probabilities of nucleotides
    probT = 0;
    probC = 0;
    probG = 0;

    probAA = 0; //probabilities of bigrams
    probAT = 0;
    probAC = 0;
    probAG = 0;
    probTA = 0;
...
string DNA:: calculations (string fileName)
{
    ...
    charCount += countA + countT + countC + countG;
    ...
    '\n' << "NUCLEOTIDE PROBABILITIES:\n\tA: " << setprecision(4) << fixed << probA * 100 << "%\tT: "
    << setprecision(4) << fixed << probT * 100 << "%\tC: " << setprecision(4) << fixed << probC * 100
    << "%\tG: " << setprecision(4) << fixed << probG * 100 << "%\n\n" << "BIGRAM PROBABILITIES:\n\tAA: "
     << setprecision(4) << fixed << probAA * 100 << "%\tAT: " << setprecision(4) << fixed << probAT  * 100
     << "%\tAC: " << setprecision(4) << fixed << probAC * 100 << "%\tAG: " << setprecision(4) << fixed
     << probAG * 100<< "%\n" << "\tTA: " << setprecision(4) << fixed << probTA  * 100 << "%\tTT: "
     << setprecision(4) << fixed << probTT * 100 << "%\tTC: " << setprecision(4) << fixed << probTC  * 100
     << "%\tTG: " << setprecision(4) << fixed << probTG * 100<< "%\n" << "\tCA: " << setprecision(4)
     << fixed << probCA  * 100 << "%\tCT: " << setprecision(4) << fixed << probCT  * 100 << "%\tCC: "
      << setprecision(4) << fixed << probGC  * 100 << "%\tCG: " << setprecision(4) << fixed << probCG * 100
      << "%\n" << "\tGA: " << setprecision(4) << fixed << probGA  * 100 << "%\tGT: " << setprecision(4) << fixed
       << probGT  * 100 << "%\tGC: " << setprecision(4) << fixed << probGC  * 100 << "%\tGG: " << setprecision(4)
       << fixed << probGG * 100<< "%\n" <<endl;
    return text.str();
}
